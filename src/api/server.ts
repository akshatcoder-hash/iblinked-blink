...
app.post('/api/place-bet/:market', async (req, res) => {
  try {
    ...
    const response = await blinks.handlePlaceBetPostRequest(market, choice, amount, account, connection);
    res.json(response);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
...
app.post('/api/create-market', async (req, res) => {
    try {
      console.log('Received POST request to /api/create-market with body:', req.body);
      const { memecoinSymbol, feedSymbol, duration } = req.body;
      const account = req.body.account || req.query.account;
      
      if (!memecoinSymbol || !feedSymbol || !duration || !account) {
        console.log('Missing required parameters:', { memecoinSymbol, feedSymbol, duration, account });
        return res.status(400).json({ error: 'Missing required parameters', missingParams: { memecoinSymbol, feedSymbol, duration, account } });
      }
...
app.post('/api/resolve-market/:market', async (req, res) => {
  try {
    ...
    const response = await blinks.handleResolveMarketPostRequest(market, account, connection);
    res.json(response);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
...
app.post('/api/claim-winnings/:market', async (req, res) => {
  try {
    const { market } = req.params;
    const { account } = req.body;
    const response = await blinks.handleClaimWinningsPostRequest(market, account, connection);
    res.json(response);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
...
app.post('/api/cancel-bet/:market', async (req, res) => {
  try {
    const { market } = req.params;
    const { account } = req.body;
    const response = await blinks.handleCancelBetPostRequest(market, account, connection);
    res.json(response);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
